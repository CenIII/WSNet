...
class Diffusion(nn.Module):
    ...
    def forward(self,x): # NxDxHxW
        ...
        Hf = self.ksize
        Wf = self.ksize
        K_trans = im2col_indices(K,Hf,Wf,2,1,1) # (800,123008)
        ...
        # V_trans = im2col_indices(V,Hf,Wf,2,1,1).view(self.nheads,self.hdim,Hf*Wf,-1) # (4,8,25,123008)
        V_trans = V.permute(1,2,3,0).contiguous().view(1,self.hdim,1,H*W*N)
        # how to add back?
        out = (V_trans*att).sum(2).view(D,H,W,N).permute(3,0,1,2)
        out = self.transform(out)
        return out