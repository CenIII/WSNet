...
def get_im2col_indices(x_shape, field_height, field_width, padding=1, stride=1, dilation=1):
    # First figure out what the size of the output should be
    N, C, H, W = x_shape
    assert (H + 2 * padding / dilation - field_height) % stride == 0
    assert (W + 2 * padding / dilation - field_height) % stride == 0
    out_height = int((H + 2 * padding / dilation - field_height) / stride + 1)
    out_width = int((W + 2 * padding / dilation - field_width) / stride + 1)

    i0 = np.repeat(np.arange(0,field_height*dilation,dilation), field_width) #(9,)
    i0 = np.tile(i0, C) #(1152,)
    i1 = stride * np.repeat(np.arange(out_height), out_width) #(3844,)
    j0 = np.tile(np.arange(0,field_width*dilation,dilation), field_height * C)  #(1152,)
